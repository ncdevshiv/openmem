...
Session storage: ~/.codex/sessions/ or project .codex/
...
class CodexCliAdapter(AgentAdapter):
    """OpenAI Codex CLI specific adapter."""
    ...
    def __init__(self):
        ...
        self._codex_dir = os.path.join(os.path.expanduser("~"), ".codex")
    ...
    def get_session_messages(self, limit: int = 100) -> List[Dict[str, str]]:
        ...
        if not os.path.isdir(self._session_dir):
            return messages
        files = self.find_session_files(self._session_dir, "*.json", hours_back=168)
        for fpath in sorted(files, key=lambda f: os.path.getmtime(f), reverse=True):
