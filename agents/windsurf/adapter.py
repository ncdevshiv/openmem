...
class WindsurfAdapter(AgentAdapter):
    ...
    def inject_context(self, context: str) -> bool:
        try:
            ctx_file = os.path.join(self._workspace, ".windsurf", "memory.md")
    ...
    def get_session_id(self) -> str:
        ...
        return f"windsurf_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    def get_agent_name(self) -> str:
        return self.AGENT_NAME

    def get_skill_install_path(self) -> Optional[str]:
        return os.path.join(self._windsurf_dir, "skills", "memory")
